...
const validTo = observedAt => new Date(Date.parse(observedAt) + 7 * 86400_000).toISOString();
...
const assertion = ({ field, source_id, url, locator, observed_at, value }) => ({
  field, source_id, original_url: url,
  locator: { kind: 'document-section', value: String(locator) },
  observed_at, valid_from: observed_at, valid_to: validTo(observed_at), value_hash: sha256(value),
  verification_status: 'verified', freshness_status: 'current', conflict: false,
...
const ruleAssertions = (prefix, source, predicate, effect, observedAt) => [
  ...
  assertion({ field: `${prefix}.valid_to`, ...source, observed_at: observedAt, value: validTo(observedAt) }),
];
const makeRules = (candidate, record) => {
  ...
  const earlyEffect = { formula: typeof earlyValue === 'string' ? earlyValue : JSON.stringify(earlyValue) };
  const early = {
    ...
    valid_from: provider.observed_at, valid_to: validTo(provider.observed_at),
    field_assertions: ruleAssertions('early_termination_rules.0', providerSource, earlyPredicate, earlyEffect, provider.observed_at),
  };
  const eligibilityPredicate = { fact: 'profile.eligibility_labels', operator: 'contains', expected: candidate.extracted.join_member || '가입대상 제한 없음' };
  ...
  const eligibility = {
    ...
    valid_from: candidate.collected_at, valid_to: validTo(candidate.collected_at),
  ...
  const bonus = Math.max(0, ...candidate.extracted.options.map(option => Number(option.maximum_rate_percent) - Number(option.base_rate_percent)));
  const bonusPredicate = { fact: 'profile.preferential_conditions_verified', operator: 'eq', expected: true };
  const bonusEffect = { additional_rate_percent: Number(bonus.toFixed(6)) };
  const bonusRule = {
    rule_id: `rule.bonus.${candidate.extracted.provider_code}.${candidate.extracted.product_code}`,
    rule_type: 'bonus-rate', predicate: bonusPredicate, effect: bonusEffect, unknown_policy: 'not_applied',
    valid_from: candidate.collected_at, valid_to: validTo(candidate.collected_at),
    field_assertions: ruleAssertions('bonus_rate_rules.0', { ...fssSource, locator: `${candidate.locator.value}.spcl_cnd` }, bonusPredicate, bonusEffect, candidate.collected_at),
  };
  return { early: [early], eligibility: [eligibility], bonus: [bonusRule] };
};
...
const buildOffer = (domain, code) => {
  ...
  const options = candidate.extracted.options
    ...
    .map((option, index) => {
      const optionId = `offer.${domain}.${candidate.extracted.provider_code}.${code}.${index}`;
      const fields = [
      ...
      const output = { option_id: optionId, type: 'offer-option', term_months: option.term_months, base_rate_percent: option.base_rate_percent, maximum_rate_percent: option.maximum_rate_percent, interest_method: option.interest_method, field_assertions: fieldAssertions };
  ...
  const top = {
    deposit_protection_status: 'protected', join_channels: joinChannels,
    eligibility_rules: rules.eligibility, bonus_rate_rules: rules.bonus,
    early_termination_rules: rules.early, sales_verification_status: 'verified_active',
  };
  ...
  const provenance = [
    { source_id: candidate.source_id, original_url: candidate.original_url, source_record_id: candidate.source_record_id, locator: candidate.locator, supported_fields: ['options', 'join_channels', 'eligibility_rules', 'bonus_rate_rules', 'sales_verification_status'], collected_at: candidate.collected_at, reviewed_at: selection.reviewed_at, valid_from: candidate.collected_at, valid_to: validTo(candidate.collected_at), checksum: candidate.checksum, verification_status: 'verified', freshness_status: 'current', conflict: false },
    { source_id: providerSourceId, original_url: providerEvidence.url, source_record_id: code, locator: { kind: 'document-section', value: providerEvidence.locator }, supported_fields: [amountMinField, amountMaxField, 'early_termination_rules'], collected_at: providerEvidence.observed_at, reviewed_at: selection.reviewed_at, valid_from: providerEvidence.observed_at, valid_to: validTo(providerEvidence.observed_at), checksum: sha256(record), verification_status: 'verified', freshness_status: 'current', conflict: false },
    { source_id: protectionEvidence.source_id, original_url: protectionEvidence.url, source_record_id: code, locator: { kind: 'record', value: protectionEvidence.locator }, supported_fields: ['deposit_protection_status'], collected_at: protectionEvidence.observed_at, reviewed_at: selection.reviewed_at, valid_from: protectionEvidence.observed_at, valid_to: validTo(protectionEvidence.observed_at), checksum: sha256({ code, protectionEvidence }), verification_status: 'verified', freshness_status: 'current', conflict: false },
  ];
  return { code, missing: [], offer: {
    id: `offer.${domain}.${candidate.extracted.provider_code}.${code}.20260731`, title: `${candidate.extracted.provider} ${candidate.extracted.product_name}`,
    ...
    valid_from: observedAt, valid_to: validTo(observedAt), ...top, options, field_assertions: fieldAssertions,
...
const receipts = ['deposit', 'saving'].flatMap(domain => fs.readFileSync(path.join(KNOWLEDGE, '30-financial-products', 'banking', '_decision', `${domain}-offers.jsonl`), 'utf8').split('\n').filter(Boolean).map(JSON.parse)).map(offer => ({ receipt_id: `receipt.vertical-slice.${offer.id}`, offer_id: offer.id, observed_at: offer.observed_at, source_ids: [...new Set(offer.provenance.map(item => item.source_id))], field_assertion_count: offer.field_assertions.length + offer.options.reduce((sum, option) => sum + option.field_assertions.length, 0), option_count: offer.options.length, verification_status: 'verified', freshness_status: 'current', source_backed: true }));
