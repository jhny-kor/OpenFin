...
import { livenessPayload, readinessPayload, runtimeContract } from "../src/health.ts";
...
test("service availability never becomes a capability status", () => {
  const payload = readinessPayload({ env: { RUNTIME_VERSION: "openfin-mcp-abc123", DEPLOYMENT_COMMIT: "a".repeat(40), BUILD_TIMESTAMP: "2026-09-01T00:00:00Z", ARTIFACT_GENERATION: "generation" }, metadata: { item_count: 1, export_checksum: "x" }, manifest: { service_availability: "available", capabilities: { search: "ready", comparison: "limited" } }, artifactsLoaded: true, checksumVerified: true, manifestUrl: "https://example.test/manifest" });
...
test("readiness blocks development or incomplete runtime metadata", () => {
  assert.deepEqual(runtimeContract({ RUNTIME_VERSION: "openfin-mcp-dev" }), {
    ready: false,
    reasons: ["RUNTIME_VERSION_DEV", "BUILD_TIMESTAMP_INVALID", "DEPLOYMENT_COMMIT_MISSING", "ARTIFACT_GENERATION_MISSING"],
  });
  const payload = readinessPayload({ env: { RUNTIME_VERSION: "openfin-mcp-dev" }, metadata: { item_count: 1, export_checksum: "x" }, manifest: { service_availability: "available", capabilities: { search: "ready" } }, artifactsLoaded: true, checksumVerified: true, manifestUrl: "https://example.test/manifest" });
  assert.equal(payload.status, "blocked");
  assert.ok(payload.blocking_reasons.includes("RUNTIME_VERSION_DEV"));
});
