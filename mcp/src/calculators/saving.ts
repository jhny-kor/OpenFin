...
export function calculateSavingReturn({ monthly_payment_krw, payment_schedule_krw, payment_timing = "month_start", annual_rate_percent, term_months, tax_rate_percent = 15.4, rounding_policy }: Record<string, unknown>) {
  ...
  const taxResult = calculateInterestTax(gross_interest_krw, tax, rounding_policy);
  return taxResult && { principal_krw: principal, ...taxResult, maturity_amount_krw: roundKrw(principal + taxResult.net_interest_krw, rounding_policy), calculation_assumption: `simple_interest_with_explicit_or_fixed_schedule_paid_at_${payment_timing}` };
}
