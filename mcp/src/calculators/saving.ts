...
export function calculateSavingReturn({ monthly_payment_krw, annual_rate_percent, term_months, tax_rate_percent = 15.4 }: Record<string, unknown>) {
  ...
  return taxResult && { principal_krw: payment * months, ...taxResult, maturity_amount_krw: Math.round(payment * months + taxResult.net_interest_krw), calculation_assumption: "simple_interest_with_each_month_paid_at_month_start" };
}
