...
function assertionState(assertions: readonly RecordLike[], asOf = new Date().toISOString().slice(0, 10)) {
  ...
  const valid = assertions.filter((assertion) => verified(assertion.verification_status) && current(assertion.freshness_status) && assertion.conflict !== true && (!assertion.valid_to || String(assertion.valid_to) >= asOf) && assertion.reviewer && assertion.reviewed_at && assertion.receipt_checksum);
  const stale = assertions.filter((assertion) => !current(assertion.freshness_status) || (assertion.valid_to && String(assertion.valid_to) < asOf)).length;
...
export function evaluateEvidenceGate({ offer, option, domain }: EvidenceGateInput) {
  ...
  const state = assertionState(assertions);
  const provenance = list(offer.provenance);
  const sourceReceiptCount = provenance.filter((entry) => typeof entry.source_id === "string" && typeof entry.checksum === "string" && verified(entry.verification_status) && current(entry.freshness_status) && entry.conflict !== true).length;
  ...
  const strictSchemaValid = presentFields.length === requiredFields.length && typeof option.option_id === "string";
  ...
  const officialSourceAssertionCoverage = assertions.length ? assertions.filter((entry) => typeof entry.source_id === "string" && typeof entry.original_url === "string").length / assertions.length : 0;
  ...
  const comparisonApproved = strictSchemaValid && salesActive && protectionReview && sourceReceiptCount > 0 && assertions.length >= requiredFields.length && state.verified === assertions.length && state.conflicts === 0 && state.stale === 0 && state.missingReceipt === 0;
  ...
  const comparisonApprovedWithPromotion = comparisonApproved && promotionPresent && promotion.comparison_approved === true;
