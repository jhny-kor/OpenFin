...
function currentItem(item: Record<string, unknown>): boolean {
  ...
  return item.verification_status === "verified" && item.recommendation_status === "verified_recommendation_candidate" && item.recommendation_scope === "public_recommendation" && item.sales_status === "active" && isVerifiedActive(item.sales_verification_status) && item.freshness_status === "current" && provenance.length > 0 && (!strict || assertions.length > 0) && provenance.every((entry) => {
...
export function evaluateReleaseGate({ manifest, checksumVerified = false, domain = null, item = null, deploymentCommit, now = Date.now() }: GateInput): ReleaseGateResult {
  ...
  if (manifest.recommendation_enabled && (!approval.approval_id || approval.mode !== "public" || approval.generation_id !== manifest.generation_id)) reasons.push("PUBLIC_APPROVAL_RECEIPT_INVALID");
  ...
  const domainReady = !domain || (domainState.status === "limited_public_ready" && Number(domainState.public_candidate_count ?? 0) >= requiredCount && domainState.recommendation_mode === "public");
