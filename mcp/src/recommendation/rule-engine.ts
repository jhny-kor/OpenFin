import { registeredFact, trustedCriticalFact, validateFactSources } from "./facts.ts";
...
export function evaluateRules(rules: readonly StructuredRule[], facts: Record<string, unknown>) {
  validateFactSources(facts.fact_sources);
  ...
  for (const rule of rules) {
    ...
    if (factNames.some(fact => !registeredFact(fact) || !trustedCriticalFact(facts, fact))) { unknown.push(rule.rule_id); continue; }
