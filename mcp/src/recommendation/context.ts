...
const FACT_KEYS = new Set(["as_of", "monthly_net_income_krw", "essential_monthly_expenses_krw", "liquid_assets_krw", "investment_assets_krw", "tax_rate_percent", "can_transfer_salary", "can_use_card", "can_set_auto_transfer", "is_new_customer"]);
...
export function normalizeRecommendationContext(input: Record<string, unknown> = {}, domain?: RecommendationDomain): RecommendationContext {
  ...
  const facts = pick({ ...object(nested.facts), ...legacyProfile, ...object(input.facts) }, FACT_KEYS, "facts");
  ...
  const as_of = date(nested.as_of ?? input.as_of ?? facts.as_of, "as_of");
...
export function contextFacts(context: RecommendationContext): Record<string, unknown> {
  return { ...context.facts, ...Object.fromEntries(Object.entries(context.goal).filter(([, value]) => value != null).map(([key, value]) => [`goal.${key}`, value])), ...Object.fromEntries(Object.entries(context.hard_constraints).map(([key, value]) => [`constraint.${key}`, value])) };
}
