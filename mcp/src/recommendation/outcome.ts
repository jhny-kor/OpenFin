...
export function calculateFinancialOutcome(item: RecordLike, preferences: RecordLike = {}) {
  ...
  const earlyRate = resolveEarlyTerminationRate(item, plannedTerminationMonths ?? undefined, rate.attainable_rate_percent ?? rate.base_rate_percent ?? undefined);
  ...
  const earlyTermination = isSaving || plannedTerminationMonths === null ? null : calculateEarlyTerminationReturn({ principal_krw: preferences.principal_krw ?? preferences.deposit_amount_krw, annual_early_termination_rate_percent: earlyTerminationRate, elapsed_months: plannedTerminationMonths, tax_rate_percent: tax });
  ...
  return {
    ...
    financial_outcomes: {
      ...
      early_termination: { annual_rate_percent: earlyTerminationRate, elapsed_months: plannedTerminationMonths, outcome: earlyTermination, limitations: plannedTerminationMonths === null ? ["planned termination time is required"] : earlyTerminationRate === null ? ["verified early-termination rate is unavailable"] : [] },
    },
    ...
    uncertainty: { unknown_conditions: [...rate.unknown_conditions, ...(earlyRate.reason ? [earlyRate.reason] : [])], rate_scenario: rate.scenario, early_termination_rate_known: earlyRate.status === "known", planned_termination_known: plannedTerminationMonths !== null },
  };
}
