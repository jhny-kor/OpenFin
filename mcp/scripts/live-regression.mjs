...
for (let attempt = 1; attempt <= metadataAttempts; attempt += 1) {
  try {
    ...
    if (typeof candidateManifest.generation_id !== "string" || candidateHealth.deployment_commit !== candidateManifest.openfin_120_live_regression?.deployment_commit) throw new Error("deployment/manifest generation metadata is missing or inconsistent");
    if (candidateHealth.generation_id !== candidateManifest.generation_id) throw new Error("health/manifest generation mismatch");
    if (candidateManifest.generation_id !== candidateManifest.openfin_120_live_regression?.generation_id) throw new Error("manifest live evidence generation mismatch");
