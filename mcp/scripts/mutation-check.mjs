...
const assertion = (field, overrides = {}) => ({ assertion_id: `assertion.mutation.${field}`, field, source_id: "source.mutation", original_url: "https://example.com/mutation", verification_status: "verified", freshness_status: "current", conflict: false, reviewer: "mutation-reviewer", reviewer_role: "compliance_reviewer", reviewer_permission: "quality:review", reviewed_at: "2026-07-31T00:00:00Z", reviewer_signature: "sig-mutation", receipt_id: field === "sales_verification_status" ? "receipt.sales.mutation" : undefined, receipt_checksum: `sha256:${field.padEnd(64, "0").slice(0, 64)}`, ...overrides });
const promotionIds = () => [...offerFields, ...optionFields].map((field) => `assertion.mutation.${field}`).sort();
const offer = () => ({ type: "deposit-offer", deposit_protection_status: "protected", join_channels: ["web"], eligibility_rules: [], bonus_rate_rules: [], early_termination_rules: [{}], sales_verification_status: "verified_active", sales_status: "active", source_listing_status: "listed", schema_validation_receipt: { validation_status: "valid", content_checksum: "sha256:" + "a".repeat(64) }, field_assertions: offerFields.map((field) => assertion(field)), provenance: [{ source_id: "source.mutation", original_url: "https://example.com/mutation", checksum: "sha256:source", verification_status: "verified", freshness_status: "current", reviewer: "mutation-reviewer", reviewer_role: "compliance_reviewer", reviewer_permission: "quality:review", reviewed_at: "2026-07-31T00:00:00Z", reviewer_signature: "sig-mutation" }] });
const option = () => ({ option_id: "mutation.option.12", term_months: 12, base_rate_percent: 2, maximum_rate_percent: 3, interest_method: "simple", schema_validation_receipt: { validation_status: "valid", content_checksum: "sha256:" + "b".repeat(64) }, field_assertions: optionFields.map((field) => assertion(field)), promotion_receipt: { comparison_approved: true, checksum_verified: true, comparison_profile: "user_fit_comparison", comparison_mode: "user_fit", mode: "shadow", schema_content_checksum: "sha256:" + "b".repeat(64), assertion_sets: { comparison: { assertion_ids: promotionIds() }, recommendation: { assertion_ids: promotionIds() } }, sales_verification_receipt_id: "receipt.sales.mutation", reviewer: "mutation-reviewer", reviewer_role: "compliance_reviewer", reviewer_permission: "quality:review", reviewer_signature: "sig-mutation" } });
const defaultSourceRegistry = new Map([["source.mutation", { authority_class: "provider_official" }]]);
const gate = (changedOffer = offer(), changedOption = option(), sourceRegistry = defaultSourceRegistry) => evaluateEvidenceGate({ offer: changedOffer, option: changedOption, domain: "deposit", asOf: "2026-07-31", sourceRegistry });
